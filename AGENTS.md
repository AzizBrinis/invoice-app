...
Copy `.env.example` to `.env` for local work, adjusting PostgreSQL `DATABASE_URL` + SMTP settings per environment. A legacy SQLite snapshot lives at `prisma/prisma/dev.db` for reference only—Prisma now targets Postgres everywhere (Vercel Postgres or local Docker). Rotate `SESSION_COOKIE_SECRET` and mail credentials regularly and never commit them. After every `schema.prisma` change, regenerate the client with `npm run prisma:generate` (or let `npm run vercel-build` handle it in CI) to prevent runtime mismatches.
