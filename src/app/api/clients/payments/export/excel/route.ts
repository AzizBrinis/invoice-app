...
import { generateClientPaymentsExcelForUser } from "@/server/excel";
...
export async function GET(request: Request) {
  try {
    ...
    const buffer = await generateClientPaymentsExcelForUser(tenantId, {
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        "Content-Disposition":
          'attachment; filename="paiements-clients.xls"',
      },
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { message: (error as Error).message ?? "Impossible de générer l’export Excel." },
      { status: 500 },
    );
  }
}
