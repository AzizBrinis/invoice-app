...
} from "@/app/(app)/site-web/avis-produits/actions";
import {
  getProductReview,
...
export default async function ProductReviewDetailPage({
  ...
  const redirectBase = `/site-web/avis-produits/${review.id}`;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <Button asChild variant="secondary">
            <Link href={"/site-web/avis-produits" as Route}>Retour à la liste</Link>
          </Button>
        </div>
      </div>
        ...
        <aside className="card h-fit space-y-4 p-6">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Seuls les avis approuvés sont visibles sur la fiche produit et dans les données SEO produit.
            </p>
          </div>
          {review.moderatedAt ? (
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
